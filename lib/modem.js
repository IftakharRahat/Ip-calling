 ...
 * API: POST http://<phone-ip>:8082/
 * Auth: Plain token in Authorization header (no Bearer prefix)
 * Body: { "to": "+880XXXXXXXXXX", "message": "text" }
 */
  ...
  try {
    const res = await fetch(`${config.traccar.baseUrl}/`, {
      ...
      headers: {
        ...
        'Authorization': config.traccar.token,
      },
      body: JSON.stringify({
        to: phone,
        message: message,
    ...
    return { success: false, error: `Traccar error (${res.status}): ${errorText}` };
  } catch (err) {
