 ...
 * API: GET http://<phone-ip>:8082/api/sms?phone=X&message=Y
 */
async function sendViaTraccar(phone, message) {
  try {
    ...
    const res = await fetch(`${config.traccar.baseUrl}/api/sms?${params}`, {
      method: 'POST',
      headers: {
...
/**
 * Check Traccar SMS Gateway status.
 */
async function getTraccarStatus() {
  try {
    const res = await fetch(`${config.traccar.baseUrl}/api/status`, {
      headers: {
        'Authorization': `Bearer ${config.traccar.token}`,
      },
    ...
    return { connected: false, gatewayMode: 'traccar', error: `HTTP ${res.status}` };
  } catch (err) {
