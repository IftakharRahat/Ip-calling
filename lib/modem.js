 ...
 * 
 * Supports four gateway modes:
 * 1. simulate  — Fake sends for development
 * 2. traccar   — Traccar SMS Gateway Android app (recommended)
 * 3. android   — Generic Android SMS Gateway app
 * 4. hilink    — Huawei E303 HiLink HTTP API
 */
...
export async function sendSMS(phone, message) {
  switch (config.gatewayMode) {
    case 'traccar':
      return sendViaTraccar(phone, message);
    case 'android':
...
// ═══════════════════════════════════════════
// TRACCAR SMS GATEWAY (Recommended)
// ═══════════════════════════════════════════

/**
 * Send SMS via Traccar SMS Gateway Android app.
 * API: POST http://<phone-ip>:8082/api/send  (JSON body with token)
 */
async function sendViaTraccar(phone, message) {
  try {
    const res = await fetch(`${config.traccar.baseUrl}/api/send`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${config.traccar.token}`,
      },
      body: JSON.stringify({
        to: phone,
        message: message,
      }),
      signal: AbortSignal.timeout(config.traccar.timeout),
    });
    ...
    const res2 = await fetch(`${config.traccar.baseUrl}/api/sms`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${config.traccar.token}`,
    ...
    return { success: false, error: `Traccar error (${res2.status}): ${errorText}` };
  } catch (err) {
    return { success: false, error: `Phone unreachable: ${err.message}` };
  }
}

/**
 * Check Traccar SMS Gateway status by trying to reach the phone.
 */
async function getTraccarStatus() {
  try {
    // Try to reach the Traccar app — any response means it's running
    const res = await fetch(`${config.traccar.baseUrl}`, {
      signal: AbortSignal.timeout(5000),
    });

    // Any response (even 404) means the app is running
    return {
      connected: true,
      gatewayMode: 'traccar',
      ...
      signalStrength: 100,
      simStatus: 'Ready',
      gatewayUrl: config.traccar.baseUrl,
    };
  } catch (err) {
    return { connected: false, gatewayMode: 'traccar', error: err.message };
  }
}

// ═══════════════════════════════════════════
// ANDROID SMS GATEWAY (Generic)
// ═══════════════════════════════════════════
